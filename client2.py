...
import flwr as fl
import numpy as np
import High_uility_itemset as default_hui
        ...
        for i in extensionsOfP:
            ...
            if(sum(iutils_list) >= self.minutil):
                if self.minutil_pc:
                    ...
                    # self.hui_list = np.concatenate(self.hui_list,np.array([i,sum(iutils_list)/self.total_db_util]))
                else:
                    # self.hui_list = np.concatenate(self.hui_list,np.array((i,sum(iutils_list))))
...
transactions = [(1,[('a',3),('b',2)]),(2,[('a',1)]),(3,[('a',7),('b',1),('c',2)])]
...
class FlowerClient(fl.client.NumPyClient):
    def __init__(self,highUtilityItemset) -> None:
        super().__init__()
        self.highUtilityItems = highUtilityItemset
    
    def get_parameters(self):
        # print(self.highUtilityItems)
        for item in self.highUtilityItems:
            item[0] = np.array(item[0])
            item = np.array(item)
        # print(np.array(self.highUtilityItems))
        return self.highUtilityItems

    def fit(self, parameters, config):
        print("I am in the fit\n\n\n")
        highUtilityItemset.extend(parameters)
        # print(self.highUtilityItems)
    ...
    def evaluate(self, parameters, config):
        ...
        # print(new_list)
        # default_hui.highUtilityItemset= new_list
        new_transactions =  [(1,[('a',3),('b',2)]),(2,[('a',1)]),(3,[('c',1),('b',10)]),(4,[('a',7),('b',1),('c',2)])]
        unit_profit = {'a': 4,'b': 2,'c': 7}
        # new_transactions = td.get_transactional_data()
        # unit_profit = up.get_unit_price()
        res = []
        obj = default_hui.FindHighUtilityItement(new_transactions,unit_profit,0.3,res,minutil_pc=False)
        res = obj.run_find_high_utility_itement()
        # print(res,"/n/n Paramerts: ",new_list)
        tp,fn = 0,0
        res_vaiables = []
        new_list_variable = []
        for row in res:
            res_vaiables.append(row[0])
        for row in new_list:
            new_list_variable.append(row[0])
        for i in res_vaiables:
            if i in new_list_variable:
                tp +=  1
            else:
                fn += 1
        
        # print(tp,fn)
        return 0.0, len(transactions), {"accuracy": tp/(tp+fn)}

fl.client.start_numpy_client(server_address="localhost:"+str(sys.argv[1]),grpc_max_message_length = 1024*1024*1024, client=FlowerClient(highUtilityItemset))